...
length_corr = np.array([0.0])

# Noise
noise_ratio = np.array([0.0]) # Noise strength in units of the chemical potential difference
noise_psd_ampl = np.array([1e-3]) # Amplitude of the power spectral density (note: A = 1e-7 corresponds roughly to a variance of 1e-5)
...
freq_sin = np.array([0.06]) # Frequency
...
if flag_saveFigs == True:
    pl.savefig("{}/diabErr_white_c.pdf".format(dirname_plots), format='pdf',bbox_inches=option_bboxinches)
