"""
Plot for diabatic error: white noise
Average diabatic error vs. protocol time, vary noise amplitude, high frequency cut-off above minimum gap, large protocol time
"""
...
noise_psd_ampl = np.array([1e-4, 1e-2, 1.0]) # Amplitude of the power spectral density (note: A = 1e-7 corresponds roughly to a variance of 1e-5)
...
# Data equalization
x_data_ref = np.linspace(100.0, 4000.0, 40)[::2]/tau_0
...
for ind_data in range(0, nbr_datasets):
    ...
    pl_avg, = ax.plot(x_data, y_data/noise_psd_ampl[ind_data], "-" + list_markers[index_marker], markersize=size_marker, linewidth=size_linewidth, color=list_colors[ind_data], zorder=99-ind_data, alpha=alpha)
...
ax.set_ylabel(r"$\mathcal{P}/A_{\mathrm{WN}}$", fontsize=size_axislabel_y, labelpad=8)
...
if scale_x == "log":
    ...
    #ax.xaxis.set_major_locator(mpl.ticker.FixedLocator(locs = np.logspace(-9, 0, 10)))
    ax.xaxis.set_major_locator(mpl.ticker.FixedLocator(locs = [1, 5, 10, 20, 40]))
...
if flag_legend == True:
    ...
    # Activate legend
    ax.legend(loc=loc_legend, fontsize=size_legend, framealpha=0.75, edgecolor="Black", fancybox=False, markerscale=1.0, borderaxespad=1.0, ncol=1, frameon=False)
...
# Other options
#pl.tight_layout()
pl.subplots_adjust(left=0.25, bottom=0.3)

# Save figure
if flag_saveFigs == True:
    pl.savefig("{}/diabErr_white_b_inset.pdf".format(dirname_plots), format='pdf',bbox_inches=option_bboxinches)
