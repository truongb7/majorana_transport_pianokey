...
Average diabatic error vs. noise amplitude, vary high frequency cutoff
"""
...
noise_psd_ampl = np.round(np.array([7.49894209e-05]),6) # Amplitude of the power spectral density (note: A = 1e-7 corresponds roughly to a variance of 1e-5)
w_cutoff_low = np.array([1e-8]) # Low frequency cutoff
w_cutoff_high = np.array([0.00633, 0.0633, 0.633]) # High frequency cutoff
...
# Data equalization
x_data_ref = np.logspace(-4, 4, 50)[::2]
...
flag_legend = True
size_legend = 15
loc_legend = (0.60, 0.04)
size_leg_handle = 2.5
leg_labels_manual = [
 '$\omega_{\mathrm{h}}/\Delta_{R} = 0.1$',
 '$\omega_{\mathrm{h}}/\Delta_{R} = 1$',
...
dict_xlabels = {"TAU":r"$\tau$", "STIMES":r"$\tau_{\mathrm{i}} / \tau$", "STIMEM":r"$\tau_{\mathrm{m}} / \tau$", "SLOPE":r"Slope $\alpha$", "WCL":r"$\omega_{{\mathrm{l}}}$", "WCH":r"$\omega_{{\mathrm{h}}}$", "NR":r"$r_{{\mathrm{n}}}$", "NA":r"$A_{{\mathrm{{WN}}}}$", "W":r"$\omega$", "PH":r"$\phi$", "DR":r"$r_{{\mathrm{{d}}}}$", "LC":r"$\xi$"}
dict_ylabels = {"diabErr":r"$\mathcal{P}$", "diabErr_cc":r"$\mathcal{P}_{\mathrm{cc}}$", "diabErr_ct":r"$\mathcal{P}_{\mathrm{ct}}$"}
...
# Plot a line
amp = 10
ax.plot(x_data, amp*x_data, linestyle="--", markersize=size_marker, linewidth=size_linewidth, color="Black", zorder=100, label=r"$\propto A_{\mathrm{WN}}$")
...
if flag_saveFigs == True:
    pl.savefig("{}/diabErr_white_d.pdf".format(dirname_plots), format='pdf',bbox_inches=option_bboxinches)
