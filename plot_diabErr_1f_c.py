"""
Plot for diabatic error: 1/f noise
Average diabatic error vs. high frequency cutoff, comparison with single mode "noise"
"""
...
flag_relfreqs = True # Plot relevant frequencies corresponding to relevant energies
...
# Time
tau = np.array([1520.0]) # Protocol time
...
# Noise
noise_ratio = np.array([0.05]) # Noise strength in units of the chemical potential difference
noise_psd_ampl = np.array([1e-4]) # Amplitude of the power spectral density (note: A = 1e-7 corresponds roughly to a variance of 1e-5)
w_cutoff_low = np.array([1e-8]) # Low frequency cutoff
w_cutoff_high = np.round(np.array([0.126576]),6) # High frequency cutoff
...
dirname_dataManual = "data_compare"
...
dict_rescale_symbols = {"tau":r"\tau_{{\mathrm{{LZ}}}}", "disorder_ratio":"w", "length_corr":"R", "w_cutoff_high":"\Delta_{{R}}", "freq_sin":"\Delta_{{R}}"}
...
# Data equalization
x_data_ref = np.logspace(-3, 1, 50)[::2]/mingap_clean[0]
...
flag_legend = True
size_legend = 15
loc_legend = (0.04, 0.70)
size_leg_handle = 2.5
leg_labels_manual = [
 '$1/f$ noise',
 'Single mode',
 '$\omega_{\mathrm{h}}/\Delta_{R} = 10$']
"""
...
# String
txt_str = r"$\omega_{{\mathrm{{h}}}}/\Delta_{{R}} = {:.0f}$".format(w_cutoff_high[0]/mingap_clean[0])
...
for ind_data in range(0, nbr_datasets):
    ...
    """
    # Remove outliers from x and y data
    if flag_removeOutliers == True:
        if ind_data == 0:
            x_data = list_df_data_set[ind_data][vary_column].drop(labels=[3,20])
            y_data = list_df_data_set[ind_data][column_selected].drop(labels=[3,20])
        if ind_data == 1:
            x_data = list_df_data_set[ind_data][vary_column].drop(labels=[2])
            y_data = list_df_data_set[ind_data][column_selected].drop(labels=[2])
    """
...
# Plot a line
#amp = 2e2
#ax.plot(x_data, amp*x_data, linestyle="--", markersize=size_marker, linewidth=size_linewidth, color="Black", zorder=100, label=r"$\propto \tau$")
...
if flag_saveFigs == True:
    pl.savefig("{}/diabErr_1f_c.pdf".format(dirname_plots), format='pdf',bbox_inches=option_bboxinches)
