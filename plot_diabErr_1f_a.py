...
Average diabatic error vs. protocol time, vary noise amplitude, high frequency cut-off below minimum gap
"""
...
w_cutoff_high = np.round(np.array([0.00633]),6) # High frequency cutoff
...
# String
txt_str = r"$\omega_{{\mathrm{{h}}}}/\Delta_{{R}} = {:.1f}$".format(w_cutoff_high[0]/mingap_clean[0])
...
if flag_saveFigs == True:
    pl.savefig("{}/diabErr_1f_a.pdf".format(dirname_plots), format='pdf',bbox_inches=option_bboxinches)
