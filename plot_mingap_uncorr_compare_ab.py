"""
Plot for minimum gap statistics: uncorrelated disorder 
Minimum gap average and standard deviation vs. disorder strength
"""
...
# Display analytical expressions for the average minimum gap and standard deviation
flag_plot_expr_mingap_avg = True
flag_plot_expr_mingap_var = True
...
# Disorder
disorder_ratio = np.array([0.0]) # Disorder strength (default: in units of the chemical potential difference)
...
flag_legend = True
size_legend = 20
loc_legend_avg = (0.04, 0.04)
loc_legend_var = (0.5, 0.04)
...
for ind_data in range(0, nbr_datasets):
    ...
    # Plot analytical expression for the average minimum gap
    # -> Variant "C" (see pk_functions for more details)
    if flag_plot_expr_mingap_avg == True:
        
        # Variables
        #k1 = np.pi + np.pi/lp_temp
        n1 = 1
        
        # Clean gap
        #mingap_clean = pk.bulkenerg(k1, w_temp, w_temp, Delta_temp)
        #mingap_clean = Delta_temp*np.pi/lp_temp
        #mingap_clean = 2.0*list_df_data_set[ind_data][column_selected][0]
        
        # Exact clean gap at bulk criticality condition via exact diagonalization
        arr_mu_temp = np.concatenate((np.full(L_temp, muLeft_temp), np.full(R_temp, w_temp)))
        arr_w_temp = np.full(L_temp+R_temp, w_temp)
        arr_Delta_temp = np.full(L_temp+R_temp, Delta_temp)
        #mingap_clean = 2.0*pk.diag_ham_kit(L_temp+R_temp, arr_mu_temp, arr_w_temp, arr_Delta_temp)[0][L_temp+R_temp+n1]
        
        # Exact clean gap at effective criticality condition
        # -> Note the necessary factor of 2 multiplied on data
        dirname_mingap_managed = "data_mingap/clean"
        dict_specifications_clean = {key:dict_specifications[key] for key in dict_specifications.keys()}
        dict_specifications_clean["disorder"] = "NONE"
        mingap_clean = 2.0*pk.load_mingaps(dict_specifications_clean, dict_protocol, dict_disorder, dict_noise, dict_other, dirname_mingap_managed)
        
        # Average given by the second order correction
        expr_avg = pk.expr_mingap_2ndCorr_avg_C(L_temp, R_temp, lp_temp, muLeft_temp, w_temp, w_temp, Delta_temp, n1, disorder_type, sigma_temp, xi_temp)
        
        # Shift or rescale if requested
        y_data_mingap_avg_expr = mingap_clean + expr_avg
        if yaxis_shift_values_avg.shape[0] != 0:
            y_data_mingap_avg_expr = y_data_mingap_avg_expr + yaxis_shift_values_avg[ind_data]
        if yaxis_rescale_values_avg.shape[0] != 0:
            y_data_mingap_avg_expr = y_data_mingap_avg_expr*yaxis_rescale_values_avg[ind_data]

        # Define x-data
        if vary_param_choice == "DR":
            if flag_std == True:
                x_data_mingap_avg_expr = sigma_temp
            else:
                x_data_mingap_avg_expr = list_df_data_set[ind_data][vary_column]
        elif vary_param_choice == "LC":
            #x_data_mingap_avg_expr = list_df_data_set[ind_data][vary_column]
            x_data_mingap_avg_expr = x_data
        else:
            x_data_mingap_avg_expr = x_data
            
        # Plot
        ax.plot(x_data_mingap_avg_expr/w, y_data_mingap_avg_expr, "--", linewidth=size_linewidth, color="Black", zorder=15)
        #ax.plot(x_data_mingap_avg_expr/w, y_data_mingap_avg_expr, "--", linewidth=size_linewidth, color=list_colors[ind_data], zorder=15)
        #ax.plot(x_data_mingap_avg_expr, y_data_mingap_avg_expr, "--", linewidth=size_linewidth, color="C{}".format(ind_data), zorder=15)
...
if flag_legend == True:
    ...
    if flag_plot_expr_mingap_avg == True:
        ax.plot([], [], "--", linewidth=size_linewidth, color="Black", label="Perturbation theory")
...
if flag_saveFigs == True:
    pl.savefig("{}/mingap_uncorr_avg.pdf".format(dirname_plots), format='pdf',bbox_inches=option_bboxinches)
...
for ind_data in range(0, nbr_datasets):
    ...
    # Plot analytical expression for the variance
    # -> Variant C (see pk_functions for more details)
    if flag_plot_expr_mingap_var == True:
        
        # Variance given by average of square of first order correction
        n1 = 1
        expr_var = pk.expr_mingap_1stCorrSqr_avg_C(L_temp, R_temp, lp_temp, muLeft_temp, w_temp, w_temp, Delta_temp, n1, disorder_type, sigma_temp, xi_temp)
        
        # Turn variance into standard deviation if requested
        y_data_mingap_var_expr = expr_var
        if flag_mingap_std == True:
            y_data_mingap_var_expr = np.sqrt(y_data_mingap_var_expr)
        
        # Shift or rescale if requested
        if yaxis_shift_values_std.shape[0] != 0:
            y_data_mingap_var_expr = y_data_mingap_var_expr + yaxis_shift_values_std[ind_data]
        if yaxis_rescale_values_std.shape[0] != 0:
            y_data_mingap_var_expr = y_data_mingap_var_expr*yaxis_rescale_values_std[ind_data]
            
        # Define x-data
        if vary_param_choice == "DR":
            if flag_std == True:
                x_data_mingap_var_expr = sigma_temp
            else:
                x_data_mingap_var_expr = list_df_data_set[ind_data][vary_column]
        elif vary_param_choice == "LC":
            #x_data_mingap_var_expr = list_df_data_set[ind_data][vary_column]
            x_data_mingap_var_expr = x_data
        else:
            x_data_mingap_var_expr = x_data
        
        # Plot
        ax_var.plot(x_data_mingap_var_expr/w, y_data_mingap_var_expr, "--", linewidth=size_linewidth, color="Black", zorder=15)
        #ax_var.plot(x_data_mingap_var_expr/w, y_data_mingap_var_expr, "--", linewidth=size_linewidth, color=list_colors[ind_data], zorder=15)
        #ax_var.plot(x_data_mingap_var_expr, y_data_mingap_var_expr, "--", linewidth=size_linewidth, color="C{}".format(ind_data), zorder=15)
...
#ax_var.set_ylabel(diabErr_column + r"/$A_{psd}$", fontsize=size_axislabel_y, labelpad=8)
...
# Save figure
if flag_saveFigs == True:
    pl.savefig("{}/mingap_uncorr_std.pdf".format(dirname_plots), format='pdf',bbox_inches=option_bboxinches)
